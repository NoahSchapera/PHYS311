...
int main() {
    ...
    for (int iter1 = 0; iter1 < maxiter; iter1++)
    {
        cout << iter1 << endl;

        for (int iter2 = 0; iter2 < maxiter; iter2++)
        {
                ...
                //cout << iter1 << " " << iter2 << " " << ecc << endl;
                file_name = "Orbit-" + to_string(MS[iter1][0]) + "-" + to_string(MS[iter2][0]) + "-" + to_string(e) +".txt";
                //to_string(to_string(MS[iter2][0])[0] + to_string(MS[iter2][0])[1] + to_string(MS[iter2][0])[2])
                //to_string(to_string(ecc)[0] + to_string(ecc)[1] + to_string(ecc)[2])
                
                ofstream outdata;
                outdata.open(file_name.c_str(), ios::out);
                if (outdata.fail()) {
                    std::cout << " Unable to open Orbit.txt --- Terminating calculation"
                        << "\n\n"
                ...
                //Write output headers
                    ...
                    //Determine (x,y) positions of centers of stars
                        ...
                        outdata << t / P << '\t' << (v1r + vcmxp) / 1000 << '\t'
                            << (v2r + vcmxp) / 1000 << '\t' << Mbol;
                        outdata << scientific;
                        outdata << '\t' << Lt * dS / S << '\t' << (y1p + vcmyp * t) / AU
                            << '\t' << (z1p + vcmzp * t) / AU << '\t' << (y2p + vcmyp * t) / AU
                            << '\t' << (z2p + vcmzp * t) / AU << '\t' << vcmyp * t / AU
                            << '\t' << vcmzp * t / AU << endl;
